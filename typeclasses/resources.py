...
def spawnJunk(caller=None):
    ...
    for n in range(0, int(results.count() * (GEM_SPAWN_PERCENT / 100))):
        loc = choice(results)
        ob = create_object(key=gem(), typeclass="typeclasses.resources.Resource", home=loc, location=loc, attributes=[('resources',{'gem': 1})])

    for n in range(0, int(results.count() * (ESSENCE_SPAWN_PERCENT / 100))):
        loc = choice(results)
        ob = create_object(key=essence(), typeclass="typeclasses.resources.Resource", home=loc, location=loc, attributes=[('resources',{'essence': 1})])
